...
frappe.ui.form.on('UK VAT Return', {
	...
	fetch_returns_hmrc: function(frm) {
		...
		frappe.call({
			"method" : "uk_vat.uk_vat.doctype.uk_vat_return.uk_vat_return.get_open_obligations",
			"args" : {
	...
	submit_to_hmrc: function(frm) {
		...
		frappe.confirm(
			...
			function(){
				...
				frappe.call({
					"method" : "uk_vat.uk_vat.doctype.uk_vat_return.uk_vat_return.submit_vat_return",
					"args" : {
						name : frm.doc.name,
